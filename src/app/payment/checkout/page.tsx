...
export default function CheckoutPage() {
  const [urlParams, setUrlParams] = useState({
    ...
    code: '',
    planName: '',
    email: ''    // ← נוספה תמיכה באימייל אם מועבר ב-URL
  })
  ...
  // נקבע success_url שיכיל כבר את כל מה שהדף /payment/success צריך
  const buildIframeUrl = () => {
    // Trial dates for success URL
    const trialStart = new Date().toISOString()
    const trialEnd = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString()

    // All params we want on /payment/success
    const successQS = new URLSearchParams({
      ...
      trial_end: trialEnd
      // NOTE: Tranzila תוסיף בעצמה &TranzilaTK=... כשמופעל מודול טוקנים
    })

    // 👉 לעבוד עם מסוף הטוקנים כדי לקבל טוקן ללא חיוב אימות
    const baseUrl = 'https://direct.tranzila.com/fxpyairsabagtok/iframenew.php'

    const params = new URLSearchParams({
      sum: '1',            // לא משמעותי ב-K, משאירים 1 ליתר ביטחון
      currency: '2',       // USD (אם במסוף לא מאופשר מט"ח, יחויב ב-ILS)
      tranmode: 'K',       // Token only, no check (ל-VK החלף ל'VK')
      nologo: '1',

      // Language
      lang: 'il',
      
      // Custom fields (יחזרו ב-notify/redirect)
      u1: urlParams.code,
      u2: urlParams.plan,
      ...
      // Success/Fail URLs (Return Method=GET בממשק!)
            ...
            <div style={{ order: 2 }}> 
              ...
              <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
                  <img src="https://www.tranzila.com/images/logo-tranzila.png" alt="Tranzila" style={{ height: '30px', opacity: 0.7 }} />
...
// עוזר לשקט נפשי ב-TypeScript/Next 15
export {}
